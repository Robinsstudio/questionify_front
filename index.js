...
app.post(/FileSave/, (req, res) => {
	const { body: { file, json } } = req;
	saveFile(file, json).then( () => res.status(200).end() ).catch( (err) => handleError(err, res) );
})
...
const saveFile = (file, json) => fs.writeFileAsync(join(cloud, ...file), json);
